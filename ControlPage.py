...
class RunGroup(CNCRibbon.ButtonGroup):
    def __init__(self, master, app):
        ...
        # HDO
        b = Ribbon.LabelButton(self.frame, self, "<<Scan>>",
                image=Utils.icons["origin32"],
                text=_("Scan"),
                compound=TOP,
                background=Ribbon._BACKGROUND)
        b.pack(side=LEFT, fill=BOTH)
        tkExtra.Balloon.set(b, _("Run scan"))
        self.addWidget(b)
...
#            self.grid_anchor(CENTER)
    ...
    # HDO    
    def runScan(self, event=None):
        self.app.setStatus("scanning ...")
        self.app.pendant.put("G90")
        self.app.pendant.put("G0 X[xmin] Y[ymin]")
        self.app.pendant.put("G0 X[xmax]")
        self.app.pendant.put("G0 Y[ymax]")
        self.app.pendant.put("G0 X[xmin]")
        self.app.pendant.put("G0 Y[ymin]")
        self.app.setStatus("scanning done.")
...
class StateFrame(CNCRibbon.PageExLabelFrame):
    def __init__(self, master, app):
        ...
        row += 1
        col = 0
        b = Checkbutton(f, text=_("Laser Focus"),
                image=Utils.icons["intersection"],
                command=self.laserControl,
                compound=LEFT,
                indicatoron=False,                
                variable=self.laser,
                padx=1,
                pady=0)
        tkExtra.Balloon.set(b, _("Start/Stop Laser Focus (M3 S1/M5)"))
        b.grid(row=row, column=col, pady=0, sticky=NSEW)
        self.addWidget(b)        
    ...
    # HDO
    #----------------------------------------------------------------------
    def laserControl(self, event=None):
        print "called laserControl"
        if self._gUpdate: return
        # Avoid sending commands before unlocking
        if CNC.vars["state"] in (Sender.CONNECTED, Sender.NOT_CONNECTED):
            ...
            return
        if self.laser.get():
            self.sendGCode("M3 S1")
            self.app.setStatus("Laser Focus ON")
        else:
            self.sendGCode("M5")
            self.app.setStatus("Laser Focus OFF")

    def laserControlHotkey(self, event=None):
        if self._gUpdate: return
        # Avoid sending commands before unlocking
        if CNC.vars["state"] in (Sender.CONNECTED, Sender.NOT_CONNECTED):
            print "not connected"
